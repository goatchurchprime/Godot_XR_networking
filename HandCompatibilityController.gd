...
var corresponding_handcontroller : ARVRController
var corresponding_handcontroller_remotetransform : RemoteTransform

# Called when the node enters the scene tree for the first time.
func _ready():
	corresponding_handcontroller = ARVRHelpers._get_controller(self, "XXXX", controller_id+2, NodePath(""))

	corresponding_handcontroller_remotetransform = RemoteTransform.new()
	...
	corresponding_handcontroller.connect("button_pressed", self, "_on_hand_button_pressed")
	corresponding_handcontroller.connect("button_release", self, "_on_hand_button_release")
...
func _on_hand_button_release(button : int) -> void:
	if button == XRTools.Buttons.VR_BUTTON_AX:
		emit_signal("button_release", XRTools.Buttons.VR_TRIGGER)
...
func _get_configuration_warning():
	...
	if chand == null:
		return "Missing ARVRController.controller_id = %d" % (controller_id+2)
	return ""

var prevactive = false
func get_is_active() -> bool:
	var res = .get_is_active()
	if res != prevactive:
		prevactive = res
...
func get_joystick_axis(pickup_axis_id : int) -> float:
	if corresponding_handcontroller.get_is_active():
		...
		return 0.0
	return .get_joystick_axis(pickup_axis_id)
