...
func download(plugin):
	...
	if plugin.url.ends_with(".zip"):
		result = exec.download_zip(plugin.url, global_dest_dir, plugin.branch, plugin.tag)
	else:
...
class _GitExecutable extends RefCounted:
	...
	func download_zip(url, dest, ldir, lfilt): # url = zip file url, dest = folder to extract it into (folder name will match zip file name)
		...
		for file_path in files:
			...
			if lfilt and not file_path.starts_with(lfilt):
				continue
