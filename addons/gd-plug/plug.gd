...
func download(plugin):
	...
	if plugin.url.ends_with(".zip"):
		result = exec.download_zip(plugin.url, global_dest_dir, plugin.branch)
	else:
...
class _GitExecutable extends RefCounted:
	...
	func download_zip(url, dest, ldir): # url = zip file url, dest = folder to extract it into (folder name will match zip file name)
